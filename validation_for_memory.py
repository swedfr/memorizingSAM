...
from segment_anything.modeling.ImageEncoder3D_m import ImageEncoderViT3D
...
parser.add_argument('-tdp', '--test_data_path', type=str, default='/content/drive/MyDrive/lighting_sam_3d/VisualizeMemory/data')
parser.add_argument('-vp', '--vis_path', type=str, default='/content/drive/MyDrive/lighting_sam_3d/VisualizeMemory/Savedir/finetunememory')
...
parser.add_argument('-mp',"--memorypath",type = str,default = '/content/drive/MyDrive/lighting_sam_3d/ckpt/FastSAM3DMemory')
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None,distance = None,index = None):
    ...
    with torch.no_grad():
        image_embedding,times = sam_model_tune.image_encoder(img3D.to(device),distance,index) # (1, 384, 16, 16, 16)
    print("time" + str(times))
    image_embedding = image_embedding[-1]
    FLOPS = profile(sam_model_tune.image_encoder,(img3D.to(device),distance,index,))[0] 
    print("Flops" + str(FLOPS))
    # end_time = time.time() 
    # encoder_time += (end_time - start_time) #
    memory_before = torch.cuda.max_memory_allocated(device) 
    print("memory" + str(memory_before)) #
    torch.cuda.reset_max_memory_allocated(device)
    for num_click in range(num_clicks):
        #
        with torch.no_grad():
...
if __name__ == "__main__":   
    ...
    for i in range(1,len(os.listdir(args.memorypath)) + 1):
      kv = torch.load(os.path.join(args.memorypath,str(i),'image.pt'))
      kv = kv.detach().cpu().numpy()
      kv = kv.flatten().reshape(1,d)
      knn.add(kv)
    ...
    memoryencoder = ImageEncoderViT3D(
            depth=6,
            embed_dim=768,
            img_size=128,
            mlp_ratio=4,
    ...
    if(args.dim==3):
        ...
        sam_model_tune.image_encoder = memoryencoder
    ...
    for batch_data in tqdm(test_dataloader):
        for i in range(0,1):
            ...
            q = image3D.detach().cpu().numpy()
            q = q.flatten().reshape(1,d)
            distance,index = knn.search(q,3)
            ...
            if(1 == 0):
                ...
                if(args.dim==3):
                    seg_mask_list, points, labels, iou_list, dice_list,t,decoder_time,memory_before, memory_decoder, FLOPS = finetune_model_predict3D(
                        ...
                        prev_masks=None,distance = distance,index = index)
                elif(args.dim==2):
