...
class ImageEncoderViT3D(nn.Module):
    def __init__(
        ...
        skip_layer = 2,
        memorylayer = 4,
        memorypath = "/content/drive/MyDrive/lighting_sam_3d/ckpt/FastSAM3DMemory",
    ) -> None:
        """
        Args:
    ...
    def forward(self, x: torch.Tensor,distance,index) -> torch.Tensor:
        t = time.time()
        ...
        for blk in self.blocks:
            x = blk(x,i,self.memorylayer,distance,index,self.memorypath)
            i += 1   
        # x = [1,16,16,16,768]
...
class Block3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor,i,memorylayer,distance,index,memorypath) -> torch.Tensor:
        shortcut = x
        x = self.norm1(x)
        # Window partition
        ...
        x = self.attn(x,i,memorylayer,distance,index,memorypath)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor,i,memorylayer,distance,index,memorypath) -> torch.Tensor:
        ...
        if i == memorylayer:
          x = x * 0.3
          for j in range(0,3):
            ratiobase = self.calculate_ratio(distance[0],0.7)
            mk = torch.load(os.path.join(memorypath,str(index[0][j]+1),"layer" + str(i),"key.pt"))
            mv = torch.load(os.path.join(memorypath,str(index[0][j]+1),"layer" + str(i),"value.pt"))
            mattn = (q * self.scale) @ mk.transpose(-2, -1)        
            if self.use_rel_pos:
              mattn = add_decomposed_rel_pos(mattn, q, self.rel_pos_d, self.rel_pos_h, self.rel_pos_w, (D, H, W), (D, H, W))

            mattn = mattn.softmax(dim=-1)
            mx = (attn @ mv).view(B, self.num_heads, D, H, W, -1).permute(0, 2, 3, 4, 1, 5).reshape(B, D, H, W, -1)
            x = x + distance[0][j] / ratiobase * mx
        x = self.proj(x)
        return x

    def calculate_ratio(self, matrix,bodyratio):
      ratiobase = 0
      for i in range(0,matrix.shape[0]):
        ratiobase += bodyratio / matrix[i]
      return ratiobase
