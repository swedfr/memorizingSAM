...
parser.add_argument('-cp', '--checkpoint_path', type=str, default='/content/drive/MyDrive/lighting_sam_3d/ckpt/FastSAM3D.pth')
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    ...
    FLOPS = profile(sam_model_tune.image_encoder,(img3D.to(device),))[0] 
    print(times)
    print("flops" + str(FLOPS))
    # FLOPs for image encoder part
    # end_time = time.time() 
